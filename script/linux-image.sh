...
main() {
    ...
    *)
    ...
    if [[ "${arch}" == "${dpkg_arch}" ]]; then
        ...
        mkdir -p "${libgcc_root}"
        pushd "${libgcc_root}"
        apt -d --no-install-recommends download "${libgcc_packages[@]}"
        popd
    fi
    ...
    # Add Debian keys.
    curl --retry 4 -sSfL 'https://ftp-master.debian.org/keys/archive-key-{7.0,8,9,10,11}.asc' -O
    curl --retry 4 -sSfL 'https://ftp-master.debian.org/keys/archive-key-{8,9,10,11}-security.asc' -O
    curl --retry 4 -sSfL 'https://ftp-master.debian.org/keys/release-{7,8,9,10,11}.asc' -O
    curl --retry 4 -sSfL 'https://www.ports.debian.org/archive_{2020,2021,2022,2023,2024}.key' -O
    ...
    cd "/qemu/${arch}"
    apt -d --no-install-recommends download \
        ${deps[@]+"${deps[@]}"} \
        "busybox:${arch}" \
        "${dropbear}:${arch}" \
        "libtommath1:${arch}" \
    ...
    if [[ "${arch}" != "${dpkg_arch}" ]]; then
        ...
        for package in "${libgcc_packages[@]}"; do
            ...
            apt -d --no-install-recommends download "${package}=${version}"
        done
        ...
        # since apt relies on libgcc_s1 and libstdc++6.
